...
Select P.FirstName , P.SecondName , P.SureName , P.LicenseNo,P.ShortName , @TotalH+TBF_Total_Hours+((TBF_Total_Mins+@TotalMin)/60) GrandTotalHours ,@Total_TBF_H+TBF_Total_Hours + ((@Total_TBF_M+TBF_Total_Mins)/60)  as TBF_Total_Hours, @TotalMin+TBF_Total_Mins-(((TBF_Total_Mins+@TotalMin)/60)*60) GrandTotalMin, @Total_TBF_M+TBF_Total_Mins - (((@Total_TBF_M+TBF_Total_Mins)/60)*60) as TBF_Total_Mins,@TotalMonthH MonthTotalH, @TotalMonthM MonthTotalMin, Ap.Name, A.IATACode, A2.IATACode , S.FlightNo, R.IsHeavy, R.ReportDate, AP.AirPlaneID, S.ChoxOff, S.ChoxOn, P.IsPilot,
...
Select R.ReportDate, A.IATACode [From], A2.IATACode [To], R.FlightNo
from Flight R
Inner Join Sector S on S.ReportID = R.ReportID
Inner Join AirPort A on S.From_AirportID = A.AirPortID
Inner Join AirPort A2 on S.To_AirportID = A2.AirPortID
Inner Join FlightPilot RP on R.ReportID = RP.ReportID
Where RP.PilotID = @PilotID And 
...
Declare @StartDate DateTime ,
		@EndDate DateTime  
Select @StartDate = Convert (DateTime,'09/10/2012', 101)
Select @EndDate = Convert (Datetime,'09/18/2012',101)
*/
...
Inner Join FlightPilot RP on F.ReportID = RP.ReportID 
inner join Pilot P on RP.PilotID = P.PilotID 
...
--Left Join Pilot P on RP.PilotID = P.PilotID
--Left join Position PP on RP.PositionID = PP.PositionID
inner Join FlightCrew RC on R.ReportID = RC.ReportID
