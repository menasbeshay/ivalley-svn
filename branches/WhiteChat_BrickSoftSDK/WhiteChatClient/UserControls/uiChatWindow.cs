...
namespace WhiteChatClient.UserControls
{
    public partial class uiChatWindow : UserControl
    {
        ...
        delegate void HandleIncomingMsgCallback(object sender, string buddy, string message);
        #endregion
        ...
        public uiChatWindow()
        {
            ...
            CurrentUser.Client.OnMessage += new OnMessageEventHandler(Client_OnMessage);
        ...
        #region Events 
        void Client_OnMessage(object sender, string buddy, string message)
        {
            HandleIncomingMsg(sender, buddy, message);
        }

        private void HandleIncomingMsg(object sender, string buddy, string message)
        {
            if (!this.InvokeRequired)
            {
                ...
                {
                    int _index;
                    if ((_index = message.IndexOf(item.Key)) > -1)
                    {
                        message.Replace(item.Key, item.Value);
                    }
                }
                uiRichTextBoxHistory.SelectedRtf = message;
            }
            else
            {
                HandleIncomingMsgCallback b = new HandleIncomingMsgCallback(HandleIncomingMsg);
                this.Invoke(b, new object[] { sender, buddy, message });
            }
        }
        ...
        private void uiRichTextBoxMsg_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    CurrentUser.Client.SendMessage(currentChatUser, uiRichTextBoxMsg.Text);
                    foreach (var item in CurrentUser.Emotions)
                    {
                        int _index;
        ...
        public void LoadBuddies()
        {
            ...
            {
                foreach (DictionaryEntry buddy in CurrentUser.Client.BuddyList)
                {
                    YahooBuddy yb = (YahooBuddy)buddy.Value;
