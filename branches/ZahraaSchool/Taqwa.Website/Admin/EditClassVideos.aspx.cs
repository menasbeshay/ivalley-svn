...
{
    public partial class EditClassVideos : System.Web.UI.Page
    {
        public int CurrentAttachment
        {
            get
            {
                if (Session["CurrentAttachment_v"] != null && !string.IsNullOrEmpty(Session["CurrentAttachment_v"].ToString()) && Session["CurrentAttachment_v"].ToString() != "0")
                {
                    return Convert.ToInt32(Session["CurrentAttachment_v"].ToString());
                }
                else
                {
                    return 0;
            ...
            set
            {
                Session["CurrentAttachment_v"] = value;
            }
        }
        ...
        protected void uiButtonUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                db.UpdateClassAttachment(CurrentAttachment, classid, yearid, courseid, uiTextBoxArName.Text, uiTextBoxEnName.Text, uiTextBoxArTeacherName.Text, uiTextBoxEnTeacherName.Text,DateTime.Now,uiTextBoxURL.Text, true);
            }
            else
            {
                db.AddClassAttachment(classid, yearid, courseid, uiTextBoxArName.Text, uiTextBoxEnName.Text, uiTextBoxArTeacherName.Text, uiTextBoxEnTeacherName.Text, DateTime.Now, uiTextBoxURL.Text, true);
            }
            uiPanelCurrentClasses.Visible = true;
            uiPanelCurrent.Visible = false;
            ClearFields();
        ...
        private void BindData()
        {
            ...
            int.TryParse(uiDropDownListSchoolYear.SelectedValue, out yearid);
            int.TryParse(uiDropDownListCourses.SelectedValue, out courseid);
            ds = db.GetGetClassAttachmentByClassIDAndYearIDAndCourseIDAndType(classid,yearid,courseid, true);
