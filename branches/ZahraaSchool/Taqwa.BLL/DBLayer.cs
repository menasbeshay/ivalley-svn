...
namespace Taqwa.BLL
{
    ...
    {       
        ...
        public bool AddOutStanding(string ENName, string ARName, string PicturePath, DateTime Date, int ClassID, string ENComments, string ARComments)
        {
            int rows = 0;
            try
            {
                rows = SqlHelper.ExecuteNonQuery(ConnectionString, "AddOutStanding", ENName, ARName, PicturePath, DateTime.Now, ClassID,ENComments, ARComments);
                return (rows > 0);
            }
            catch (Exception)
            {
        ...
        public bool UpdateOutstanding(int OutstandingsID, string ENName, string ARName, string PicturePath, DateTime Date, int ClassID, string ENComments, string ARComments)
        {
            int rows = 0;
            try
            {
                rows = SqlHelper.ExecuteNonQuery(ConnectionString, "UpdateOutstanding", OutstandingsID, ENName, ARName, PicturePath, DateTime.Now, ClassID, ENComments, ARComments);
                return (rows > 0);
            }
            catch (Exception)
            {
