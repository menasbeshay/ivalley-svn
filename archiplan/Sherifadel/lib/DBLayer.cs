...
namespace archiplan
{
    public class DBLayer
    {
        ...
        public bool SetPageContent(int PageID, string Content,string title)
        {
            bool result = false;
            if (!string.IsNullOrEmpty(title))
                command.CommandText = string.Format("Update Pages Set Content = '{0}', Title='{1}' where ID = {2}", Content, title, PageID);
            else
        ...
        public bool SetPageContent(int PageID, string Content, string title, string Content_en, string title_en)
        {
            bool result = false;
            if (!string.IsNullOrEmpty(title))
                command.CommandText = string.Format("Update Pages Set Content = '{0}', Title='{1}' , Content_en='{2}', Title_en='{3}' where ID = {4}", Content, title, Content_en, title_en, PageID);
            else
                command.CommandText = string.Format("Update Pages Set Content = '{0}', Content_en = '{1}' where ID = {2}", Content, Content_en, PageID);
        ...
        public bool SetProductContent(int ProductID, string name, string name_en, string mainpic, string thumbs, string description, string description_en, string price)
        {
            bool result = false;
            command.CommandText = "Update products Set productName = '" + name + "',productName_en = '" + name_en + "', description = '" + description + "', description_en = '" + description_en + "' , price = '" + price + "'";
            if (!string.IsNullOrEmpty(mainpic))
        ...
        public bool SetProductContent(int ProductID, string name, string mainpic, string thumbs, string description, string price)
        {
            bool result = false;
            command.CommandText = "Update products Set productName = '" + name + "', description = '" + description + "' , price = '" + price + "'";
            if (!string.IsNullOrEmpty(mainpic))
                command.CommandText += " , Picpath = '" + mainpic + "'";
        ...
        public bool AddProductContent(string name, string mainpic, int catID, string thumb, string description, string price)
        {
            bool result = false;
            command.CommandText = string.Format("insert into products (productname, PicPath, categoryID, thumbs, description, price) values ('{0}', '{1}', {2}, '{3}', '{4}' , '{5}')", name, mainpic, catID, thumb, description, price);
            try
            {
                Connect();
                int rows = command.ExecuteNonQuery();
        ...
        public bool AddProductContent(string name,string name_en, string mainpic, int catID, string thumb, string description,string description_en, string price)
        {
            bool result = false;
            command.CommandText = string.Format("insert into products (productname, PicPath, categoryID, thumbs, description, price, productname_en, description_en) values ('{0}', '{1}', {2}, '{3}', '{4}' , '{5}', '{6}', '{7}')" , name, mainpic, catID, thumb, description, price,name_en,description_en);
            try
            {
                Connect();
                int rows = command.ExecuteNonQuery();
        ...
        public DataSet GetCatNameByCatID(int catid)
        {
            ds = new DataSet();
        ...
        public bool AddItemContent(string name, string picpath, int productid)
        {
            bool result = false;
            command.CommandText = string.Format("insert into productitems (Itemname, picpath, productid) values ('{0}', '{1}', {2} )", name, picpath, productid);
            try
            {
                Connect();
                int rows = command.ExecuteNonQuery();
