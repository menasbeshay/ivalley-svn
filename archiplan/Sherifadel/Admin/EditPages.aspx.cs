    ...
    public partial class EditPages : System.Web.UI.Page
    {
        ...
        protected void uiGridViewPages_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "EditPage")
            {
                ...
                {
                    uiTextBoxTitle.Text = ds.Tables[0].Rows[0]["Title"].ToString();
                    uiTextBoxTitle_en.Text = ds.Tables[0].Rows[0]["Title_en"].ToString();
                    uiFCKeditorContent.Value = Server.HtmlDecode(ds.Tables[0].Rows[0]["Content"].ToString());
                    uiFCKeditorContent_en.Value = Server.HtmlDecode(ds.Tables[0].Rows[0]["Content_en"].ToString());
                }
                uiPanelCurrentPages.Visible = false;
                uiPanelCurrent.Visible = true;
            }
        ...
        {
            DBLayer db = new DBLayer();
            db.SetPageContent(CurrentPage, Server.HtmlEncode(uiFCKeditorContent.Value), uiTextBoxTitle.Text, Server.HtmlEncode(uiFCKeditorContent_en.Value), uiTextBoxTitle_en.Text);
            uiTextBoxTitle.Text = "";
            uiFCKeditorContent.Value = "";
            uiTextBoxTitle_en.Text = "";
            uiFCKeditorContent_en.Value = "";
            CurrentPage = 0;
