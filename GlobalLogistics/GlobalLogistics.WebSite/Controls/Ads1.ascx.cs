...
{
    public partial class Ads1 : System.Web.UI.UserControl
    {
        ...
        private void LoadLocationDefaults()
        {
            ...
            location1.LoadByPrimaryKey(1);
            uiLiteralScript.Text = "<script type=\"text/javascript\">$(document).ready(function () {$('#Ad1').cycle({ fx: 'fade' , timeout: " + location1.AdsTimeout.ToString() + " }); $('#Ad1').css('display', 'block'); });  </script>";
        }

        private void BindEnglishData()
        {
            BLL.Ads objData = new BLL.Ads ();
            objData.GetAllAdsByLocation(1);
