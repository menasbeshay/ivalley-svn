				...
				activity.runOnUiThread(new Runnable() {
					public void run() {
						...
						if (mState == State.RECORDING) {
							...
							//Log.d(TAG, "currentTimeLeft: %d", currentTimeLeft);
							...
							if (currentTimeLeft % 1000 == 0) {
								//Log.d(TAG, "currentTimeLeft mod: %d", currentTimeLeft%1000);	
...
//			Log.d(TAG, "sampleRate: %d", mSampleRate);
	...
	public static synchronized void startRecording(Activity context) {
		if (!mRecording) {
			stopPlaying();
			// disable rotation
...
//				Log.d(TAG, "not sending, deleting: %s", mSendingFile);
				new File(mFileName).delete();
			}
...
//		Log.d(TAG, "max recorded volume: %d", maxVolume);
...
////						Log.d(TAG, "ffmpeg out: %s", shellLine);
//					}
//
//					@Override
//					public void processComplete(int exitValue) {
////						Log.d(TAG, "ffmpeg complete, deleting: %s", mSendingFile);
...
////											// delete files
////											Log.d(TAG, "upload complete, deleting %s", m4aFile);
////											new File(m4aFile).delete();
////										}
////									});
//						}
//						else {
////							Log.d(TAG, "not sending message, ffmpeg error: %d", exitValue);
...
//				Log.d(TAG, "sendVoiceMessage, deleting: %s", mSendingFile);
				new File(mSendingFile).delete();
//				Utils.makeToast(activity, activity.getString(R.string.error_message_generic));
			}
		}
	...
	public synchronized static void playVoiceMessage(Context context, final SeekBar seekBar, final SurespotMessage message) {
		...
		if (!mPlaying && differentMessage) {
			...
			try {
				...
				Log.d(TAG, "playVoiceMessage error");
				playCompleted();
				return;
			}
	...
	private static void updatePlayControls() {
		...
		if (voicePlayed != null && voiceStop != null) {
			if (isCurrentMessage()) {
				Log.d(TAG, "updatePlayControls, currentMessage");
...
//					Log.d(TAG, "message: %s not playing", message);
...
//							Log.d(TAG, "setting played to visible");
//							voicePlayed.setVisibility(View.VISIBLE);
//							if (voicePlay != null) {
//								voicePlay.setVisibility(View.GONE);
//							}
//						}
//						else {
//							Log.d(TAG, "setting played to gone");
//							voicePlayed.setVisibility(View.GONE);
//							if (voicePlay != null) {
...
//				
	...
	private static void setProgress(final SeekBar seekBar, final int progress) {
		...
		seekBar.post(new Runnable() {
			...
			@Override
			public void run() {
				// Log.d(TAG, "Setting progress to %d", progress);
	...
	private static class SeekBarThread implements Runnable {
		...
		public void run() {
			...
			while (mRun) {
				...
				if (mDuration > -1) {
					...
					if (isCurrentMessage()) {
						...
						try {
							...
							Log.d(TAG, "SeekBarThread error getting current position");
							mRun = false;
							break;
						}

						progress = (int) (((float) currentPosition / (float) mDuration) * 101);
						// Log.d(TAG, "SeekBarThread: %s, currentPosition: %d, duration: %d, percent: %d", mSeekBar, currentPosition, mDuration,
						// progress);
						...
						// Log.d(TAG, "setting seekBar: %s, progress: %d", mSeekBar, progress);
				...
				try {
					...
					mRun = false;
					Log.d(TAG, "SeekBarThread interrupted");
				}
			}
		...
		public void completed() {
			Log.d(TAG, "SeekBarThread completed");
			mRun = false;
