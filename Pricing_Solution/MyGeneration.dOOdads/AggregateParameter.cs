...
namespace MyGeneration.dOOdads
{
	...
	/// prds.Query.AddResultColumn(Pricing.ColumnNames.CategoryID);
	/// prds.Query.AddResultColumn(Pricing.ColumnNames.Discontinued);
	///
	/// // If you have an Aggregate, ANSI SQL requires an AddGroupBy
	/// // for each AddResultColumn. Check your DBMS docs.
	/// prds.Query.AddGroupBy(Pricing.ColumnNames.CategoryID);
	/// prds.Query.AddGroupBy(Pricing.ColumnNames.Discontinued);
	///
	/// prds.Query.AddOrderBy(Pricing.ColumnNames.Discontinued, WhereParameter.Dir.ASC);
	/// 
