...
{
    public partial class EditTourismList : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (ParentPackage == 0)
                {
                    Response.Redirect("EditPackages.aspx");
                }
            }
        ...
        private void BindData()
        {
            ...
            uiRepeaterCurrentTourismLists.DataSource = db.GetArabicTourismlists(ParentPackage,true);
        ...
        protected void uiLinkButtonUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (CurrentList != 0)
            {
                ...
                if (temp != imagepath && string.IsNullOrEmpty(imagepath))
                    db.SetArabicTourismlistContent(CurrentList, uiTextBoxBrief.Text, Server.HtmlEncode(uiFCKeditorContent.Value), uiTextBoxTitle.Text, temp, Convert.ToInt32(uiTextBoxOrder.Text), ParentPackage,uiCheckBoxViewInWeb.Checked);
                else
                    db.SetArabicTourismlistContent(CurrentList, uiTextBoxBrief.Text, Server.HtmlEncode(uiFCKeditorContent.Value), uiTextBoxTitle.Text, imagepath, Convert.ToInt32(uiTextBoxOrder.Text), ParentPackage,uiCheckBoxViewInWeb.Checked);
            }
            else // add new
            {
                db.AddArabicTourismlistContent(uiTextBoxBrief.Text, Server.HtmlEncode(uiFCKeditorContent.Value), uiTextBoxTitle.Text, imagepath, Convert.ToInt32(uiTextBoxOrder.Text),ParentPackage,uiCheckBoxViewInWeb.Checked);
            }
            uiPanelViewLists.Visible = true;
            uiPanelEditTourismList.Visible = false;
            ClearFields();
        ...
        protected void uiRepeaterTourismLists_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "EditList")
            {
                ...
                uiCheckBoxViewInWeb.Checked = (bool)ds.Tables[0].Rows[0]["arViewInWeb"];
