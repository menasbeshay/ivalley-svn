...
namespace archiplan
{
    public partial class Projects_en : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                if ((Request.QueryString["pid"] != null && !string.IsNullOrEmpty(Request.QueryString["pid"])))
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        uiLabelProducCat.Text = ds2.Tables[0].Rows[0]["CategoryName_en"].ToString();
                        DataSet pics = db.GetAllItemsByProductID(Convert.ToInt32(Request.QueryString["pid"].ToString()));

                        uiLabelProductName.Text = ds.Tables[0].Rows[0]["ProductName_en"].ToString();
                        uiLiteralDescription.Text = Server.HtmlDecode(ds.Tables[0].Rows[0]["Description_en"].ToString());
                        uiRepeaterProjectPics.DataSource = pics;
                        uiRepeaterProjectPics.DataBind();
                    }
                    else
                    {
                        Response.Redirect("projects_en.aspx");
                    }
                }
        ...
        private void BindProducts()
        {
            ...
            if (ds2.Tables[0].Rows.Count > 0)
            {
                ...
                uiLabelCategory.Text = ds2.Tables[0].Rows[0]["CategoryName_en"].ToString();
        ...
        {
            Response.Redirect("projects_en.aspx?cid=" + Request.QueryString["cid"].ToString());
        }
    }
}