IF OBJECT_ID('dbo.GetMemberTypeSpecID') IS NOT NULL
  DROP FUNCTION GetMemberTypeSpecID
GO

CREATE FUNCTION GetMemberTypeSpecID
   (@MemberID INT)
RETURNS INT
AS
BEGIN
   DECLARE @MemberTypeSpecID INT
   SELECT TOP 1 @MemberTypeSpecID=MemberTypeSpec.ID 
   FROM Member
	INNER JOIN MemberType ON MemberType.MemberID=Member.MemberID
	INNER JOIN MemberTypeSpecDuration ON MemberTypeSpecDuration.ID=MemberType.MemberTypeSpecDurationID
	INNER JOIN MemberTypeSpec ON MemberTypeSpec.ID=MemberTypeSpecDuration.MemberTypeSpecID
   WHERE (MemberType.EndDate>=GETDATE() OR MemberType.EndDate IS NULL)
   ORDER BY MemberType.StartDate DESC
	
  IF @MemberTypeSpecID IS NULL
	SET @MemberTypeSpecID=1
  
  RETURN @MemberTypeSpecID	
...
CREATE PROCEDURE proc_MailAdsSend
AS
BEGIN

DECLARE @senderID INT
SELECT @senderID=MemberID FROM Member WHERE Name=N'«· ”ÊÌﬁ Ê«·≈⁄·«‰'
DECLARE @mailSubject NVARCHAR(1000)
SET @mailSubject=N'≈⁄·«‰«  «·„Êﬁ⁄'
...
BEGIN   
	   ...
	   BEGIN
		...
		SET @membersNames=@membersNames+N'Õ”«»«  «· —ﬁÌ…';
		INSERT INTO @receipents(MemberID)
		SELECT MemberID FROM Member WHERE (SELECT dbo.GetMemberTypeSpecID(MemberID))>1
	   END
	   ...
	   BEGIN
		IF(LEN(@membersNames)>0)
			SET @membersNames=@membersNames+',';
		SET @membersNames=@membersNames+N'Õ”«»«  „„Ì“…';
		--INSERT INTO @receipents(MemberID)
		--SELECT MemberID FROM Member WHERE (SELECT dbo.GetMemberTypeSpecID(MemberID))>1
	   END
