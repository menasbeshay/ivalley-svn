...
public class RehearsalAudioRecorder {
	...
	private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener() {
		public void onPeriodicNotification(AudioRecord recorder) {
			...
			try {
				...
				Log.d(RehearsalAudioRecorder.class.getName(),  "Error occured in updateListener, recording is aborted");
				stop();
			}
		}
		...
		try {
			...
			if (rUncompressed) { // RECORDING_UNCOMPRESSED
				...
				if (bufferSize < AudioRecord.getMinBufferSize(sampleRate, channelConfig, audioFormat)) { // Check to make sure buffer size is not smaller than
					...
					Log.d(RehearsalAudioRecorder.class.getName(), "Increasing buffer size to " + Integer.toString(bufferSize));
				}
			...
			if (e.getMessage() != null) {
				Log.d(RehearsalAudioRecorder.class.getName(),  e.getMessage());
			}
			else {
				Log.d(RehearsalAudioRecorder.class.getName(),  "Unknown error occured while initializing recording");
			}
			state = State.ERROR;
		}
	}
	...
	public void setOutputFile(String argPath) {
		try {
			...
			if (e.getMessage() != null) {
				...
				Log.d(RehearsalAudioRecorder.class.getName(),  "Unknown error occured while setting output path");
			}
			state = State.ERROR;
		}
	}
	...
	public void prepare() {
		try {
			if (state == State.INITIALIZING) {
				if (rUncompressed) {
					if ((aRecorder.getState() == AudioRecord.STATE_INITIALIZED) & (fPath != null)) {
						...
						Log.d(RehearsalAudioRecorder.class.getName(), "prepare() method called on uninitialized recorder");
						state = State.ERROR;
					}
				}
				else {
				...
				Log.d(RehearsalAudioRecorder.class.getName(), "prepare() method called on illegal state");
				release();
				state = State.ERROR;
			}
		}
			...
			if (e.getMessage() != null) {
				...
				Log.d(RehearsalAudioRecorder.class.getName(),  "Unknown error occured in prepare()");
			}
			state = State.ERROR;
		}
	}
		...
		if (state == State.RECORDING) {
			...
			if ((state == State.READY) & (rUncompressed)) {
				try {
					...
					Log.d(RehearsalAudioRecorder.class.getName(),  "I/O exception occured while closing output file");
				}
	...
	public void start() {
		if (state == State.READY) {
			...
			Log.d(RehearsalAudioRecorder.class.getName(), "start() called on illegal state");
			state = State.ERROR;
		}
	}
	...
	public void stop() {
		if (state == State.RECORDING) {
			...
			Log.d(RehearsalAudioRecorder.class.getName(), "stop() called on illegal state");
			state = State.ERROR;
		}
	}
