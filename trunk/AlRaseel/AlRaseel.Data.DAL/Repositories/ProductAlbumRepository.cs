...
{
    public class ProductAlbumRepository
    {
        #region Variables
        DBDataContext db;
        public ProductAlbum _Obj { get; set; }
        #endregion
        #region Methods
        public ProductAlbumRepository()
        {
            db = new DBDataContext();
            _Obj = new ProductAlbum();
        }
        ...
        public Boolean Edit(String ID, String ModifiedBy)
        {
            try
            {
                _Obj = db.ProductAlbums.FirstOrDefault(pram => pram.Id == new Guid(ID));
                if (_Obj != null)
                {
        ...
        public DataTable LoadByProductIdAndLanguageId(String LanduageId,String ProductId,Boolean ActiveState = true, String SortField = "CreatedOn", String SortType = "ASC")
        {
            try
            {
                ...
                var Query = (from pram in db.ProductAlbums
                             where pram.Active == ActiveState
                             && pram.LanguageId == new Guid(LanduageId)
        ...
        public ProductAlbum LoadById(String ID)
        {
            if (ID != null)
            {
                _Obj = db.ProductAlbums.FirstOrDefault(pram => pram.Id == new Guid(ID));
