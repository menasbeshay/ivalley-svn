...
var report_suite = "unica-golsitenovodev";
if (window.location.href.indexOf("compre2.voegol.com.br") >= 0 || window.location.href.indexOf("www.voegol.com.br") >= 0) {
  report_suite = "unica-gol-prod";
}

var s_account=report_suite;
...
s.linkInternalFilters="javascript:,voegol.com.br"
...
function s_doPlugins(s) {
	...
	var campaignTmp = s.getPreviousValue(s.campaign,'gpv_v0','');
	if (campaignTmp != '' && campaignTmp != 'no value') {
		s.prop18 = campaignTmp;
		s.prop19 = campaignTmp + ':' + s.pageName;
	}

	var icampaignTmp = s.getPreviousValue(s.eVar40,'gpv_v40','');
	if (icampaignTmp != '' && icampaignTmp != 'no value') {
		s.prop20 = icampaignTmp;
		s.prop21 = icampaignTmp + ':' + s.pageName;
	}

	var iduserTmp = s.getPreviousValue(s.prop16,'gpv_c16','');
	if (iduserTmp != '' && iduserTmp != 'no value') {
		s.prop22 = iduserTmp + ':' + s.pageName;
	}

	var tipouserTmp = s.getPreviousValue(s.prop17,'gpv_c17','');
	if (tipouserTmp != '' && tipouserTmp != 'no value') {
		s.prop23 = tipouserTmp + ':' + s.pageName;
	}

	var landingTmp = s.getPreviousValue(s.eVar41,'gpv_v41','');
	if (landingTmp != '' && landingTmp != 'no value') {
		s.prop24 = landingTmp + ':' + s.pageName;
	}
	...
	if (s.getQueryParam('s_emid')) {
		if (!s.campaign) {
			s.campaign=s.getQueryParam('s_emid');
			s.prop18 = s.campaign;
			s.prop19 = s.campaign + ':' + s.pageName;
		}
	}

	if (s.getQueryParam('s_icid')) {
		if (!s.eVar40) {
			s.eVar40 = s.getQueryParam('s_icid');
			s.prop20 = s.eVar40;
			s.prop21 = s.eVar40 + ':' + s.pageName;
		}
	}
	...
	if (s.getQueryParam('s_kwcid')) {
		...
		/*linhas para capturar o email dentro do parametro identificado por id_mail_mkt*/
		s.getQueryParam('id_mail_mkt') 
		s.eVar67 = s.getQueryParam('id_mail_mkt');
		
	if (s.prop16 > 0) {
		s.prop22 = s.prop16 + ':' + s.pageName;
	}

	if (s.prop17 != "") {
		s.prop23 = s.prop17 + ':' + s.pageName;
	}

	if (s.eVar41 != "") {
		s.prop24 = s.eVar41 + ':' + s.pageName;
	}
...
//	s.setupFormAnalysis();
	...
	s.eVar40   = s.getValOnce(s.eVar40, "gvo_v40", 0);
	s.eVar41   = s.getValOnce(s.eVar41, "gvo_v41", 0);

	/*Populate Time Parting prop and eVar */
	var temphr = s.getTimeParting('h', '-3', new Date().getFullYear()); // Set hour 
	var tempday = s.getTimeParting('d', '-3', new Date().getFullYear()); // Set day
	var tempweek = s.getTimeParting('w', '-3', new Date().getFullYear()); // Set Weekend / Weekday

	if (temphr) {
	  s.prop27 = tempweek + " : " + tempday + " : " + temphr;
	  s.eVar43 = s.prop27;
	}
	...
	if (s.prop26 && !s.eVar44) s.eVar44 = s.prop26;
	if (s.prop29 && !s.eVar47) s.eVar47 = s.prop29;
	if (s.prop30 && !s.eVar48) s.eVar48 = s.prop30;
	if (s.prop31 && !s.eVar49) s.eVar49 = s.prop31;
	if (s.prop32 && !s.eVar50) s.eVar50 = s.prop32;
	if (s.purchaseID && !s.transactionID) s.transactionID = s.purchaseID;


	if (s.eVar36 && s.eVar37) {
	  s.eVar38 = s.eVar36 + '-' + s.eVar37;
	}
...
// Pluggin Captura dados Automaticos pela URL
function remove_acentos_aunica(frase){
	var x;
	var y;
	var chars = ["а","и","м","т","щ","в","к","о","ф","ы","д","л","п","ц","ь","б","й","н","у","ъ","г","х","А","И","М","Т","Щ","В","К","О","Ф","Ы","Д","Л","П","Ц","Ь","Б","Й","Н","У","Ъ","Г","Х","?"];
  	var codes =   ["a","e","i","o","u","a","e","i","o","u","a","e","i","o","u","a","e","i","o","u","a","o","A","E","I","O","U","A","E","I","O","U","A","E","I","O","U","A","E","I","O","U","A","O"," "];
	for(x = 0; x <= chars.length; x++){
		var plin = frase[x];	
		for(y=0; y <= chars.length; y++){
			if(chars[y] == plin){
				frase = frase.replace(chars[y], codes[y]);
...
function PageLoadAunica(){
	var completa = decodeURI(window.location.toString());
	var dominio = completa.split("/");
	/*A variavel dominio[2] pega o dominio do site*/
	var pos_ini = completa.indexOf(dominio[2]);
	var tamanho_comp = completa.length;
	var completa = completa.substr(pos_ini,tamanho_comp);
	var completa = completa.toLowerCase();
	var quebra = completa.split("/");
	var i;
	var j = 1;
	var tam_quebra = quebra.length;
	var pageName = "";
	var channel = "";
	var pageTitle = "";
	var pageContent = "";
	...
	if (quebra[1] == "smiles" || quebra[1] == "gollog"){
		channel = quebra[1];
		if (quebra[1] == "gollog"){
			pais = "pt-br";
		}else{
	...
	for (i = 2; i <= tam_quebra; i++){
		if (channel == "smiles" && i == 2){
			i++;
		}
		if (quebra[i] != "" && quebra[i] != undefined){
			if (quebra[i] != "paginas" && quebra[i] != "Paginas"){
				j++;
				sections[j] = sections[j-1]+":"+quebra[i];
				pageName = sections[j];
			}else{
				i++;
			}
			if (quebra[i].indexOf("default.aspx") > -1 && j > 1){
				sections[j]="";
			}else if (quebra[i].indexOf(".aspx") > -1){
				title_edt = quebra[i].substr(0,quebra[i].indexOf(".aspx"));
				title_edt = title_edt.toLowerCase();
				title_edt = remove_acentos_aunica(title_edt);
				pageName += ":"+title_edt;
			}
		}
	}
	...
	if (channel != "smiles"){
		if(j == 1){
			pageName = sections[1]+":home";
		}else if (sections[j-1] == sections[j-2]){
			pageName = sections[j-2]+":home";
		}
	}else{
		if (sections[j-2] == undefined){
			pageName = sections[1]+":home";
		}else if (sections[j-1] == sections[j-2]){
			pageName = sections[j-2]+":home";
		}
	...
	if (sections[1] == undefined){sections[1] = "";}
	if (sections[2] == undefined){sections[2] = "";}
	if (sections[3] == undefined){sections[3] = "";}
	if (sections[4] == undefined){sections[4] = "";}

	var patt1=/:/g;

	/* Atribuiзгo dos valores das variбveis do sitecatalyst */
	s.pageName=pais+":"+pageName;
	s.channel=channel;
	s.prop31=pais;
	s.hier1 = pageName.replace(patt1,"|");
	s.prop44 = sections[1];
	s.prop45 = sections[2];
	s.prop46 = sections[3];
	s.prop47 = sections[4];
...
/*fim do plugin captura dados URL*/


/* WARNING: Changing any of the below variables will cause drastic
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
s.visitorNamespace="aunica"
s.trackingServer="aunica.112.2o7.net"
