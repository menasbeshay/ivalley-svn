...
namespace MyGeneration.dOOdads
{
	...
	abstract public class DynamicQuery
	{
		...
		/// {
		/// 	Pricing prds = new Pricing();
		///
		/// 	// Note we only bring back these two columns for performance reasons, why bring back more?
		/// 	prds.Query.AddResultColumn(Pricing.ColumnNames.ProductID);
		/// 	prds.Query.AddResultColumn(Pricing.ColumnNames.ProductName);
		///
		/// 	// Sort
		/// 	prds.Query.AddOrderBy(Pricing.ColumnNames.ProductName, MyGeneration.dOOdads.WhereParameter.Dir.ASC);
		/// 	
		/// 	// Load it
		/// 	prds.Query.Load();
		///
